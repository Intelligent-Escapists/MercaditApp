...
def confirmar_email(token):
    ...
    usuario = model.buscar_usuario_por_correo(correo)

    if usuario is None:
        return jsonify({"error": "Usuario no encontrado"}), 404

    status = model.confirmar_email(usuario)
...
def login_usuario():
    correo = request.json["correo"]
    password = request.json["password"]
    usuario = model.validar_usuario(correo, password)
...
def existeUsuario(id_usuario):
    return model.buscar_usuario_por_id(id_usuario=id_usuario)
